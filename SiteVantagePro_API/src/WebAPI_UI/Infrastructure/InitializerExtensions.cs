...
public class ApplicationDbContextInitializer(ILogger<ApplicationDbContextInitializer> logger, ApplicationDbContext context, UserManager<ApplicationUser> userManager, RoleManager<ApplicationRole> roleManager)
{
    ...
    public async Task TrySeedAsync()
    {
        ...
        if (_userManager.Users.All(u => u.UserName != superadmin.UserName))
        {
            ...
            var newSuperAdminRole = await _roleManager.FindByNameAsync(superadminRole.Name!);
            var allClaims = await _roleManager.GetClaimsAsync(newSuperAdminRole!);//ToDo
            var allPermissions = Permissions.GeneratePermissionsForModule("Products");
            foreach (var permission in allPermissions)
            {
                if (!allClaims.Any(a => a.Type == "Permission" && a.Value == permission))
                {
                    await _roleManager.AddClaimAsync(newSuperAdminRole!, new Claim("Permission", permission));
                }
            }
        }
        else
        {
            if (_userManager.Users.All(u => u.UserName == superadmin.UserName))
            {
                //--https://codewithmukesh.com/blog/permission-based-authorization-in-aspnet-core/#Seeding_Users_and_Roles
                //await _roleManager.SeedClaimsForSuperAdmin();
                var newSuperAdminRole = await _roleManager.FindByNameAsync(superadminRole.Name!);
                if (newSuperAdminRole != null)
                {
                    var allClaims = await _roleManager.GetClaimsAsync(newSuperAdminRole!);
                    var allPermissions = Permissions.GeneratePermissionsForModule("Products");
                    foreach (var permission in allPermissions)
                    {
                        if (!allClaims.Any(a => a.Type == "Permission" && a.Value == permission))
